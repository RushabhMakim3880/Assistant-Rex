...
class PrinterAgent:
    ...
    def __init__(self, profiles_dir: str = "printer_profiles"):
        ...
        self._error_tracker = set()
        self.gemini_client = None # Set by server
        self.sio = None # Set by server
        self.monitoring_tasks = {} # host -> task
        ...
        self.printers = {} # host -> Printer object
    ...
    def _detect_slicer_path(self) -> Optional[str]:
        ...
        print("[PRINTER] Warning: No Slicer (Orca/Prusa) found. Please install OrcaSlicer or PrusaSlicer to enable 3D model slicing.")
        return None
    ...
    async def print_stl(self, stl_path: str, printer_name: str, 
        ...
        if success:
            # Start monitoring this print
            if printer.camera_url:
                asyncio.create_task(self._start_failure_detection(printer))
    ...
    async def _start_failure_detection(self, printer: Printer):
        """Starts a background loop to watch for printer failures."""
        if printer.host in self.monitoring_tasks:
            self.monitoring_tasks[printer.host].cancel()
            
        task = asyncio.create_task(self._failure_detection_loop(printer))
        self.monitoring_tasks[printer.host] = task

    async def _failure_detection_loop(self, printer: Printer):
        """Periodically grabs a frame and checks for spaghetti/failures."""
        print(f"[PRINTER] AI Failure Detection started for {printer.name}")
        while True:
            try:
                # Check status first - only monitor if printing
                status = await self.get_print_status(printer.host)
                if not status or status.state not in ["printing", "Printing"]:
                    print(f"[PRINTER] Print finished or stopped. Stopping AI detection for {printer.name}")
                    break
                
                # Grab Frame
                async with aiohttp.ClientSession() as session:
                    async with session.get(printer.camera_url) as resp:
                        if resp.status == 200:
                            # If it's a stream, we might need to parse the first frame
                            # or just use a snapshot URL if available
                            # Most printers have a /webcam/?action=snapshot for this
                            snapshot_url = printer.camera_url.replace("action=stream", "action=snapshot")
                            async with session.get(snapshot_url) as snap_resp:
                                if snap_resp.status == 200:
                                    img_data = await snap_resp.read()
                                    await self._analyze_frame(printer, img_data)
                
                await asyncio.sleep(60) # Watch every minute
            except asyncio.CancelledError:
                break
            except Exception as e:
                print(f"[PRINTER] Failure detection error: {e}")
                await asyncio.sleep(30)

    async def _analyze_frame(self, printer: Printer, img_data: bytes):
        """Uses Gemini Vision to detect print failures."""
        if not self.gemini_client: return
        
        prompt = """
        Analyze this 3D printer camera frame. 
        Are there any obvious failures like 'spaghetti' (loose filament), 
        the part being knocked off the bed, or massive warping?
        If everything looks normal, respond 'NORMAL'.
        If there is a failure, respond 'FAILURE: [Reason]'.
        """
        try:
            ...
            if "FAILURE" in result:
                print(f"[PRINTER] CRITICAL: Potential failure detected on {printer.name}: {result}")
                if self.sio:
                    await self.sio.emit('maintenance_alert', {
                        "type": "printer_failure",
                        "message": f"Possible print failure on {printer.name}: {result}. Should I pause the print?",
                        "printer_host": printer.host
                    })
        except Exception as e:
            print(f"[PRINTER] Frame analysis failed: {e}")
